...
class DebugNode(LifecycleNode):
    ...
    def on_activate(self, state: LifecycleState) -> TransitionCallbackReturn:
        ...
        self.image_sub = message_filters.Subscriber(
            self, Image, "/ugv_cam/image_raw", qos_profile=self.image_qos_profile
        )
    ...
    def draw_box(
        ...
        # ▼▼▼ 중심점 시각화 기능 추가 ▼▼▼
        center_x = round(box_msg.center.position.x)
        center_y = round(box_msg.center.position.y)
        cv2.circle(cv_image, (center_x, center_y), 2, color, -1)  # 중심에 채워진 원(점) 그리기
        # ▲▲▲ 중심점 시각화 기능 추가 ▲▲▲
    ...
    def detections_cb(self, img_msg: Image, detection_msg: DetectionArray) -> None:
        ...
        # FPS 텍스트를 이미지 좌측 상단에 표시
        fps_text = f"FPS: {self._fps:.2f}"
