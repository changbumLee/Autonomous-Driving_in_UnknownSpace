...
class YoloNode(LifecycleNode):
    ...
    def on_configure(self, state: LifecycleState) -> TransitionCallbackReturn:
        ...
        self.center_px_pub_=self.create_publisher(Point, '/yolo/center_px', 10)
    ...
    def on_activate(self, state: LifecycleState) -> TransitionCallbackReturn:
        ...
        self._sub = self.create_subscription(
            Image, "/esp32_img", self.image_cb, self.image_qos_profile
        )
    ...
    def parse_boxes(self, results: Results) -> List[BoundingBox2D]:
        ...
        if results.boxes:
            ...
            for i, box_data in enumerate(results.boxes):
                ...
                # 터미널에 감지된 객체와 중심점 좌표를 출력
                class_name = self.yolo.names[int(box_data.cls)]
                self.get_logger().info(
                    f"Detection: {class_name}, "
                    f"Center XY: ({msg.center.position.x:.1f}, {msg.center.position.y:.1f})"
                )
                center_px_point = Point()
                center_px_point.x = float(box[0])
                center_px_point.y = float(box[1])
                center_px_point.z = 0.0

                self.center_px_pub_.publish(center_px_point)

                # append msg
                boxes_list.append(msg)
            # ▲▲▲ 중심점 추출 및 로그 출력 기능 추가 ▲▲▲
